...
import {
  ...
  MessageCircle, Layers, ShoppingBag, CreditCard,
  CheckCircle2, Package, Crown, Phone
} from 'lucide-react';
...
const MODES = [
  {
    emoji: '🤖', label: 'AI Mode', color: '#3A86FF',
...
const PLANS = [
  {
    name: 'Basic', emoji: '⚡', type: 'One-time',
...
const NFC_PRODUCTS = [
  { emoji: '💳', name: 'NFC Card', price: '$12' },
  { emoji: '🔑', name: 'NFC Keychain', price: '$10' },
  { emoji: '⌚', name: 'NFC Bracelet', price: '$9' },
  { emoji: '⬡', name: 'NFC Sticker', price: '$6' },
];
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 overflow-hidden space-y-40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-7">
          {MODES.map((mode, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
              ...
              <div className="absolute top-0 right-0 w-40 h-40 rounded-full blur-[80px] opacity-0 group-hover:opacity-25 transition-opacity pointer-events-none"
      ...
      <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-neon text-brand-accent text-sm font-bold mb-5">
            <Package size={14} /> Physical + Digital
          </div>
          <h2 className="text-5xl md:text-6xl font-black tracking-tighter mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
          {NFC_PRODUCTS.map((p, i) => (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }}
              ...
              <div className="absolute inset-0 bg-brand-glow opacity-0 group-hover:opacity-5 transition-opacity" />
              <div className="text-5xl mb-4">{p.emoji}</div>
              <h3 className="font-black text-white text-base mb-1">{p.name}</h3>
              <p className="text-2xl font-black text-brand-accent">{p.price}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="p-8 md:p-12 glass-neon rounded-[40px] flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
          ...
          <Link to="/login"
            className="btn-neon text-black font-black whitespace-nowrap flex items-center gap-2">
      ...
      <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-neon text-brand-accent text-sm font-bold mb-5">
            <CreditCard size={14} /> Simple Pricing
          </div>
          <h2 className="text-5xl md:text-6xl font-black tracking-tighter mb-4">
